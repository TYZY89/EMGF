...
def tree_to_adj(sent_len, tree, directed=False, self_loop=True): #依存树存为邻接矩阵
    """
...
#begin
def inputs_to_deprel_adj(dep_heads, dep_rel, l, maxlen):  #关系弧表示矩阵
    """
    Inputs: #(B,)
    ...
    """
    # maxlen = max(l) # l => maxlen
    dep_heads= dep_heads.cpu().detach().numpy()
    dep_rel = dep_rel.cpu().detach().numpy() 
...
def head_rel_to_tree(head, rel, len_):  #关系弧
    """
    Convert a sequence of head indexes into a tree object.
    """
    rel = rel[1:len_+1].tolist()  
    head = head[1:len_+1].tolist() 
    root = None
    ...
    for i in range(len(nodes)):
        ...
        nodes[i].dist = -1  # just a filler
        if h == 0:
            root = nodes[i] # root node i=4
        else:
            nodes[h-1].add_child(nodes[i])
    
    return root

def tree_to_rel_adj(sent_len, tree, directed=False, self_loop=True): #关系弧邻接矩阵
    """
    Convert a tree object to an (numpy) dep_rel labeled adjacency matrix.
    """
    ret = np.zeros((sent_len, sent_len), dtype='float32')
        
    queue = [tree]
    idx = []
    while len(queue) > 0:
        t, queue = queue[0], queue[1:] # get first node => t

        idx += [t.idx]
        for c in t.children:
            ret[t.idx + 1, c.idx + 1] = t.rel
        queue += t.children  # children into queue
